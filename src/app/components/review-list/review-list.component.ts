...
export class ReviewListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // .where('productStatus', '==', 'waiting-for-delivery')
    // .orderBy('updateAt')
    // .limit(8)
    // ).snapshotChanges()
    // .map(actions => {
  ...
  searchBtn(keyNameSearchInput) {
    if (keyNameSearchInput != '') {
      this.firestore.collection('product').doc(keyNameSearchInput).get().toPromise()
        .then((product) => {
          ...
          if (product.data() != undefined) {
            ...
            // CHECK OWNER
            if (_product.sellerUID == this.auth.currentUserId) {
              var productShopKey: any = _product.productKey;
              // GET PRODUCT IN SHOP
              this.getProductInCollShop(productShopKey);
            }
          }
          else {
            // NOT PRODUCT KEY SEARCH BY NAME
            this.firestore.collection('product', ref => ref
              .where('name', '==', keyNameSearchInput)
              .where('productStatus', '==', 'sale-success')
              .where('sellerUID', '==', this.auth.currentUserId))
              .get().toPromise()
              .then((product) => {
                if (product.size != 0) {
                  product.forEach((doc: any) => {
                    // console.log(doc.id, " => ", doc.data());
                    this.getProductInCollShop(doc.data().productKey);
                  });
                }
                else {
  ...
  getProductInCollShop(productKey) {
    // console.log(productKey)
    this.firestore.collection('shop').doc(this.auth.currentUserId).collection('product').doc(productKey).get().toPromise()
      .then((productShop) => {
        // console.log(productShop.data())
        if (productShop.data() != undefined) {
          this.productDataFromFirebase = [{
            key: productKey,
            value: productShop.data()
          }]
          this.showTextSelectData = false;
          this.showTextNoData = false;
          this.showContent = true;
    ...
    if (this.tagEN != '' && this.dateString == '-') {
      ...
      this.firestore.collection('order', ref => ref
        ...
        .then((orderSucc) => {
          ...
          if (orderSucc.size != 0) {
            orderSucc.forEach((doc: any) => {
              // console.log(doc.id, '=>', doc.data())
              this.firestore.collection('product').doc(doc.data().productKey).get().toPromise()
              .then((product) => {
                // console.log(product.data())
                if(product.data() != undefined){
                  var _productProduct:any = product.data()
                  // CHECK PRODUCT HAVE REVIEW ?
                  this.firestore.collection('shop').doc(this.auth.currentUserId).collection('product').doc(_productProduct.productKey).collection('product-review').get().toPromise()
                    .then((productReview) => {
                      // console.log(productReview.size)
                      if (productReview.size != 0) {
                        this.productDataFromFirebase.push({
                          key: _productProduct.productKey,
                          value: doc.data()
                        });
                        this.showContent = true;
                        this.showTextSelectData = false;
  ...
  productDetail(dataKey) {
    ...
    this.router.navigate([`/review-detail/${dataKey}`]);
  }
