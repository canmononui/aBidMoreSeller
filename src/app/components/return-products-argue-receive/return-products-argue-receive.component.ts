...
@Component({
  selector: 'app-return-products-argue-receive',
  templateUrl: './return-products-argue-receive.component.html',
  styleUrls: ['./return-products-argue-receive.component.css']
})
export class ReturnProductsArgueReceiveComponent implements OnInit {
  ...
  update(argueInput){
    this.firestore.collection('order').doc(this.id).update({
      'returnProduct.sellerArgueReceiveAt': firebase.firestore.Timestamp.now(),
      'returnProduct.sellerArgueReceiveImg': this.imgProductArrayURL,
      'returnProduct.sellerArgueReceiveDes': argueInput,
      'returnProduct.status': 'sellerArgueReceive',
    })
    .then(docRef => {
      this.router.navigate([`/return-products-detail/${this.id}`]);
    })
